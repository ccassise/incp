...
class TestIncp(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_incp_src_file_does_not_exist(self):
        ...
        sender = await asyncio.create_subprocess_exec("./incp", "does_not_exist.txt", "also_does_not_exist.txt")
        await receiver.wait()
        await sender.wait()
    ...
    async def test_incp_src_file_dest_file_exists(self):
        ...
        receiver = await asyncio.create_subprocess_exec("./incp", "-l")
        sender = await asyncio.create_subprocess_exec("./incp", expected.absolute(), actual.absolute())
        await receiver.wait()
        await sender.wait()
